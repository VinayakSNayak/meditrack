...
class HealthRecordsScreen extends StatelessWidget {
  ...
  void _showAddOptions(BuildContext context) async {
    final bodySnapshot = await FirestoreService.getBodyVitalMetrics().first;
    final bloodSnapshot = await FirestoreService.getBloodMetrics().first;
    ...
    showModalBottomSheet(
      ...
      builder: (_) => Padding(
        ...
        child: ListView(
          shrinkWrap: true,
          children: [
            const Text("Body Vitals",
                style: TextStyle(fontSize: 16, fontWeight: FontWeight.w600)),
            const SizedBox(height: 12),
            ..._bodyMetrics
                .where((m) => !existingBodyTypes.contains(m))
                .map((type) => _metricOption(context, type, 'bodyVitals')),

            const SizedBox(height: 24),
            const Text("Blood Records",
                style: TextStyle(fontSize: 16, fontWeight: FontWeight.w600)),
            const SizedBox(height: 12),
            ..._bloodMetrics
                .where((m) => !existingBloodTypes.contains(m))
                .map((type) => _metricOption(context, type, 'bloodRecords')),
  ...
  Widget _metricOption(BuildContext context, String type, String category) {
    return ListTile(
      title: Text(type),
      trailing: const Icon(Icons.arrow_forward_ios, size: 16),
      onTap: () {
        ...
        Navigator.push(
          context,
          MaterialPageRoute(
            builder: (_) => AddMetricScreen(metricType: type, category: category),
          ),
        );
      },
    );
